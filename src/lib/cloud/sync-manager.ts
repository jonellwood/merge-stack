...
export function reconciliationDecision(localUpdatedAt:number,link:Awaited<ReturnType<typeof loadCloudLink>>,cloudRevision:number){
...
export async function inspectCloudState(userId:string,local:GameState){
  ...
  try{
    ...
    const decision=reconciliationDecision(local.updatedAt,link,cloud.revision);
    ...
    publish({phase:'synced',cloud,dirty:false,message:'Cloud save is current.'});return undefined;
...
async function pushLinkedSnapshot(userId:string,state:GameState){
  ...
  }catch(error){const cloud=await loadCloudSave().catch(()=>undefined);publish({phase:cloud?'conflict':'error',cloud,dirty:true,message:cloud?'Another device saved newer progress. Review both saves before continuing.':error instanceof Error?error.message:'Cloud sync failed'})}
}
