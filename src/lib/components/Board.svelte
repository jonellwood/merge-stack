...
<section class="board-wrap" aria-labelledby="board-title">
  ...
  <div class="selection-help">{selected?`Item selected — merge, move, store in the ${rackItemSize(selectedDefinition?.id??'')}U rack, or recycle.`:'Tap a workstation to generate. Select an item to move it or mount it in the Server Rack.'}</div>
