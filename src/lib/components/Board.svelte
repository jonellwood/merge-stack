<script lang="ts">
  ...
  async function useCell(index:number,item?:BoardItem){
    ...
    if(item&&itemById.get(item.definitionId)?.kind==='producer'){
      ...
      if(!produced&&gameState.player.energy<BALANCE.producerCost)showEnergyWarning();
      return
    }
      ...
      <button class:locked={cell.locked} class:occupied={!!item} class:producer={definition?.kind==='producer'} class:energy-empty={definition?.kind==='producer'&&gameState.player.energy<BALANCE.producerCost} class:denied-a={definition?.kind==='producer'&&energyWarning>0&&energyWarning%2===1} class:denied-b={definition?.kind==='producer'&&energyWarning>0&&energyWarning%2===0} class:drag-over={over===cell.index} class:merge-over={over===cell.index&&mergeTarget(cell.index)} class:selected={item?.instanceId===selected} class="cell" data-cell={cell.index} role="gridcell" aria-label={cell.locked?`Locked cell, costs ${cell.unlockCost} credits`:item?`${definition?.name}, level ${definition?.level ?? 'producer'}`:`Empty cell ${cell.index+1}`} onclick={()=>useCell(cell.index,item)}>
        ...
        {:else if item}
          <span role="presentation" class="item" class:being-dragged={dragging===item.instanceId} onpointerdown={(e)=>down(e,item)} onpointermove={move} onpointerup={up} onpointercancel={up}>
            {#if definition?.kind==='producer'}<small class="producer-label">{gameState.player.energy<BALANCE.producerCost?'ENERGY DEPLETED':'TAP TO COMPILE'}</small>{/if}
