<script lang="ts">
  ...
  import { MATCHDAY_EVENT, MATCHDAY_REDEMPTION } from '$lib/catalogs/events';
  import { discardQuote, itemsContributingToTicket, matchdayCashoutQuote } from '$lib/domain/game';
  ...
  let matchdayPayout=$state(false),tidyConfirm=$state(false),rewardSplash=$state<{icon:string;title:string;detail:string}|null>(null);
  ...
  let matchdayQuote=$derived(matchdayCashoutQuote(gameState));
  ...
  function producerLabel(item:BoardItem,producer:NonNullable<ReturnType<typeof producerByItemId.get>>){
    ...
    if(producer.activeUntil!==undefined&&clock>=producer.activeUntil)return producer.itemId==='event_pipeline'&&matchdayQuote.items?`CASH OUT · ${matchdayQuote.items}`:'ENDED';
  ...
  async function collectMatchday(){const credits=matchdayQuote.credits;if(await actions.cashoutMatchday()){matchdayPayout=false;rewardSplash={icon:'◈',title:`+${credits} credits`,detail:'Matchday items archived'};clearTimeout(rewardTimer);rewardTimer=setTimeout(()=>rewardSplash=null,2_500)}}
  ...
  async function useCell(index:number,item?:BoardItem){
    ...
    if(item&&itemById.get(item.definitionId)?.kind==='producer'){
      ...
      if(item.definitionId==='event_pipeline'&&clock>=MATCHDAY_EVENT.endsAt&&matchdayQuote.items){matchdayPayout=true;return}
    ...
    if(item?.definitionId===MATCHDAY_REDEMPTION.itemId&&clock<MATCHDAY_EVENT.endsAt){
      const now=Date.now();
      ...
      <button class:locked={cell.locked} class:occupied={!!item} class:producer={definition?.kind==='producer'} class:ticket-reserved={reservedForTicket} class:redeemable={definition?.id===MATCHDAY_REDEMPTION.itemId&&clock<MATCHDAY_EVENT.endsAt} class:infrastructure={definition?.id==='infrastructure_workbench'} class:event-pipeline={definition?.id==='event_pipeline'} class:energy-empty={!!producerConfig&&gameState.player.energy<producerConfig.energyCost} class:denied-a={definition?.kind==='producer'&&energyWarning>0&&energyWarning%2===1} class:denied-b={definition?.kind==='producer'&&energyWarning>0&&energyWarning%2===0} class:drag-over={over===cell.index} class:merge-over={over===cell.index&&mergeTarget(cell.index)} class:selected={item?.instanceId===selected} class="cell" data-cell={cell.index} role="gridcell" aria-label={cell.locked?`Locked cell, costs ${cell.unlockCost} credits`:item?`${definition?.name}, level ${definition?.level ?? 'producer'}${reservedForTicket?', contributes to a support ticket':''}${definition?.id===MATCHDAY_REDEMPTION.itemId&&clock<MATCHDAY_EVENT.endsAt?', double tap to redeem':''}`:`Empty cell ${cell.index+1}`} onclick={()=>useCell(cell.index,item)}>
        ...
        {:else if item}
          <span role="presentation" class="item" class:being-dragged={dragging===item.instanceId} onpointerdown={(e)=>down(e,item)} onpointermove={move} onpointerup={up} onpointercancel={up}>
            ...
            {#if definition?.id===MATCHDAY_REDEMPTION.itemId&&clock<MATCHDAY_EVENT.endsAt}<small class="goal-badge">GOAL!</small>{/if}
...
{#if redemption}
  <div class="goal-backdrop" role="presentation" onclick={(event)=>event.target===event.currentTarget&&(redemption=null)}>
    <div class="goal-dialog" role="dialog" aria-modal="true" aria-labelledby="goal-title">
      <button class="goal-close" onclick={()=>redemption=null} aria-label="Close reward choice">×</button><span class="goal-cup">🏆</span><small>MATCHDAY DEPLOYMENT COMPLETE</small><h2 id="goal-title">Gooooal!</h2><p>Trade your Championship Dashboard for one reward.</p>
      <div class="goal-options"><button disabled={gameState.player.energy>=gameState.player.maxEnergy} onclick={async()=>{if(await actions.redeemEvent(redemption!,'energy'))redemption=null}}><i>⚡</i><b>+{MATCHDAY_REDEMPTION.energy} Energy</b><span>{gameState.player.energy>=gameState.player.maxEnergy?'Energy already full':'Refuel production'}</span></button><button onclick={async()=>{if(await actions.redeemEvent(redemption!,'credits'))redemption=null}}><i>◈</i><b>+{MATCHDAY_REDEMPTION.credits} Credits</b><span>Fund the recovery</span></button></div>
    </div>
  </div>
{/if}
{#if matchdayPayout}
  <div class="maintenance-backdrop" role="presentation" onclick={(event)=>event.target===event.currentTarget&&(matchdayPayout=false)}><div class="maintenance-dialog event-payout" role="dialog" aria-modal="true" aria-labelledby="payout-title"><button class="maintenance-close" onclick={()=>matchdayPayout=false} aria-label="Close">×</button><span class="maintenance-icon">⚽</span><small>EVENT WRAP-UP</small><h2 id="payout-title">Full-time payout</h2><p>Matchday has ended. Archive every soccer item still on the board and collect its level-scaled value.</p><div class="payout-total"><span>{matchdayQuote.items} ITEMS</span><strong>◈ {matchdayQuote.credits}</strong><small>TOTAL CREDITS</small></div><button class="maintenance-primary" onclick={collectMatchday}>ARCHIVE &amp; COLLECT</button></div></div>
{/if}
{#if tidyConfirm}
