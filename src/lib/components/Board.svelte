...
import { discardQuote, itemsContributingToTicket, hackathonCashoutQuote } from '$lib/domain/game';
  ...
  let hackathonPayout=$state(false),tidyConfirm=$state(false),capacityPrompt=$state(false),rewardSplash=$state<{icon:string;title:string;detail:string}|null>(null);
