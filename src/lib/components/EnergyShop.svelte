<script lang="ts">
  ...
  import { shopFlavorForLevel } from '$lib/catalogs/titles';
  ...
  let flavor=$derived(shopFlavorForLevel(gameState.player.level));
...
<div class="shop-backdrop" role="presentation" onclick={(event)=>event.target===event.currentTarget&&onClose()}>
  <div class="energy-shop" role="dialog" aria-modal="true" aria-labelledby="energy-shop-title">
    ...
    <div class="shop-icon">⚡</div><small>CAPACITY MANAGEMENT</small><h2 id="energy-shop-title">Emergency Recharge</h2><p class="shop-flavor">{flavor} Restore energy to full immediately. Procurement raises the price after every purchase in the same six-hour window.</p>
