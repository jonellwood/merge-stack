...
async function commit(result: {state:GameState;ok:boolean;reason?:string;message?:string;action?:string}) {
  ...
  game.set(result.state); notice.set(result.message ?? (result.action==='merge' ? 'Merge complete' : 'Board updated')); await saveGame(result.state); return true;
}
...
export const actions = {
  ...
  devEnergy:async()=>{const state=get(game);if(!state)return;const next=structuredClone(state);next.player.energy=next.player.maxEnergy;next.player.energyUpdatedAt=Date.now();game.set(next);await saveGame(next);notice.set('Energy restored.')}
};
