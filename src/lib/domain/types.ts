...
export interface TicketTemplate { id: string; requester: string; title: string; description: string; requirements: Requirement[]; }
export interface Ticket { id: string; requesterId: string; requester: string; title: string; description: string; requirements: Requirement[]; rewards: { credits: number; xp: number }; status: 'active' | 'completed'; createdAt: number; }
export interface GameSettings { sound: boolean; reducedMotion: boolean; highContrast: boolean; }
export interface GameState { schemaVersion: number; player: PlayerState; cells: BoardCell[]; items: BoardItem[]; tickets: Ticket[]; settings: GameSettings; ticketSequence: number; updatedAt: number; }
